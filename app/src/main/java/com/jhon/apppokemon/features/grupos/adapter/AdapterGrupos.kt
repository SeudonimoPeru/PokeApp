...
class AdapterGrupos(val listener: OnClickListener) : RecyclerView.Adapter<AdapterGrupos.NoticiaViewHolder>() {
    ...
    private var items: List<Grupo> = emptyList()

    //nueva data
    fun swap(items: List<Grupo>) {
        this.items = items
        notifyDataSetChanged()
    }

    interface OnClickListener {
        fun onClick(noticia: Grupo)
    }
    ...
    class NoticiaViewHolder(val view: View) : RecyclerView.ViewHolder(view) {
        ...
        fun bind(beanGrupo: Grupo, listener: OnClickListener) {

            // se configura logica de como se compartara el  view con los datos
            with(beanGrupo) {
                txvTitle.text = nombreGrupo!!.uppercase()
                if(isAuthor==true){
                    btnEdit.visibility = View.VISIBLE
                }
                view.setOnClickListener {
                    listener.onClick(beanGrupo)
                }
            }
        }
